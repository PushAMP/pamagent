...
from pamagent.wrapper import wrap_object, ObjectProxy, callable_name
...
class CursorWrapper(ObjectProxy):
    ...
    def __init__(self, cursor, dbapi2_module, connect_params, cursor_params):
        ...
        self._pm_dbapi2_module = dbapi2_module
        self._pm_connect_params = connect_params
        self._pm_cursor_params = cursor_params

    def execute(self, sql, parameters=DEFAULT, *args, **kwargs):
        transaction = current_transaction()
        if parameters is not DEFAULT:
            with DatabaseTrace(transaction, sql, self._pm_dbapi2_module, self._pm_connect_params,
                               self._pm_cursor_params, parameters, (args, kwargs)):
                return self.__wrapped__.execute(sql, parameters, *args, **kwargs)
        else:
            with DatabaseTrace(transaction, sql, self._pm_dbapi2_module, self._pm_connect_params,
                               self._pm_cursor_params, None, (args, kwargs)):
    ...
    def executemany(self, sql, seq_of_parameters):
        ...
        if parameters is not DEFAULT:
            with DatabaseTrace(transaction, sql, self._pm_dbapi2_module, self._pm_connect_params,
                               self._pm_cursor_params, parameters):
                return self.__wrapped__.executemany(sql, seq_of_parameters)
        else:
            with DatabaseTrace(transaction, sql, self._pm_dbapi2_module, self._pm_connect_params,
                               self._pm_cursor_params):
                return self.__wrapped__.executemany(sql, seq_of_parameters)

    def callproc(self, procname, parameters=DEFAULT):
        transaction = current_transaction()
        with DatabaseTrace(transaction, 'CALL %s' % procname, self._pm_dbapi2_module, self._pm_connect_params):
            if parameters is not DEFAULT:
...
class ConnectionWrapper(ObjectProxy):
    ...
    def cursor(self, *args, **kwargs):
        return self.__cursor_wrapper__(self.__wrapped__.cursor(*args, **kwargs), self._pm_dbapi2_module,
                                       self._pm_connect_params, (args, kwargs))

    def commit(self):
        transaction = current_transaction()
        with DatabaseTrace(transaction, 'COMMIT', self._pm_dbapi2_module, self._pm_connect_params):
            return self.__wrapped__.commit()

    def rollback(self):
        transaction = current_transaction()
        with DatabaseTrace(transaction, 'ROLLBACK', self._pm_dbapi2_module, self._pm_connect_params):
...
class ConnectionFactory(ObjectProxy):
    ...
    def __call__(self, *args, **kwargs):
        ...
        with FunctionTrace(transaction, callable_name(self.__wrapped__)):
            return self.__connection_wrapper__(self.__wrapped__(*args, **kwargs), self._pm_dbapi2_module,
                                               (args, kwargs))
