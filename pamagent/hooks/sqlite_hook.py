...
class ConnectionWrapper(DBAPI2ConnectionWrapper):
    ...
    def execute(self, sql, parameters=DEFAULT):
        transaction = current_transaction()
        if parameters is not DEFAULT:
            with DatabaseTrace(transaction, sql, self._pam_dbapi2_module, self._pam_connect_params, None, parameters):
    ...
    def executemany(self, sql, seq_of_parameters):
        transaction = current_transaction()
        with DatabaseTrace(transaction, sql, self._pam_dbapi2_module, self._pam_connect_params, None,
